        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-content nopadding">
                    {!! Form::model($marka,['route'=>['markalar.update',$marka->id],'method'=>'PUT','class'=>'form-horizontal']) !!}
                    ...
                    <div class="control-group">
                        <label class="control-label">Üst Marka :</label>
                        <div class="controls">
                            <select name="ust_id">
                                <option value="0">Yok</option>
                                @foreach($markalar as $mrk)
                                    @if($marka->ust_id==$mrk->id)
                                        <option value="{{$mrk->id}}" selected>{{$mrk->adi}}</option>
                                        @else
                                            <option value="{{$mrk->id}}">{{$mrk->adi}}</option>
                                    @endif
